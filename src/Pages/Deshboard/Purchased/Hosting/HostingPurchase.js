...
const HostingPurchase = () => {
    ...
    useEffect(()=>{
        const url = `https://calm-anchorage-79518.herokuapp.com/purchasedHosting?email=${users.email}`;
         fetch(url,{
           headers:{
    ...
    return (
        ...
        <Box sx={{display:"flex",color:"#fff",bgcolor:"#8F40FB",justifyContent:"space-around",borderRadius:"30px",margin:"15px 0px",}}>
            <Link style={{textDecoration:'none',color:"#fff"}} to="/deshboard/purchasedhostings"> <Button style={{color:"#fff",fontSize:"18px"}}>hostings</Button> </Link>
            ...
            <TableContainer style={{overflowX:"visible"}} component={Paper}>
                <Table sx={{ width: "100%" }} aria-label="simple table">
                    ...
                    <TableBody>
                    {
                        hostings.map(hosting => <TableRow key={hosting._id}
                            style={{color:"red"}}
                            ...
                            <TableCell sx={{padding:"10px"}} align="left">${hosting.price}USD</TableCell>
                            <TableCell sx={{padding:"10px"}} align="left">{hosting.time} year</TableCell>
...
export default HostingPurchase;